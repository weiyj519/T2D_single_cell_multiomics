...
"""
Script name: 00_train_rna_xgboost_borutashap.py
...
Example command: python scripts/02_ml_feature_selection/00_train_rna_xgboost_borutashap.py
"""
...
class PairwiseXGBSHAPAnalyzerCore:
    ...
    def run_boruta_stability(
        ...
        for fold, (tr_idx, va_idx) in enumerate(
            ...
            boruta_params["n_estimators"] = 400

            boruta_model = XGBClassifier(**boruta_params)
...
# =========================================================
# All cell type ROC utilities
# =========================================================
def normalize_celltype_name(s: str) -> str:
    return s.replace(" ", "_").replace("/", "_")


def build_stem_to_celltype(parquet_path: str) -> dict:
    cts = (
        pd.read_parquet(parquet_path, columns=["celltype"])["celltype"]
        .dropna()
        .unique()
        .tolist()
    )
    m = {}
    for ct in cts:
        stem = normalize_celltype_name(ct)
        if stem in m and m[stem] != ct:
            print(f"[WARN] stem : {stem} -> {m[stem]} / {ct}")
        else:
            m[stem] = ct
    return m
...
def prepare_pair_split(
    data_cell: pd.DataFrame, negative_class: str, positive_class: str
):
    data_pair = data_cell[
        data_cell["condition"].isin([negative_class, positive_class])
    ].copy()
    if data_pair["condition"].nunique() < 2:
        return None
...
def gene_filter_like_original(
    X_train: pd.DataFrame, X_test: pd.DataFrame
):
    n_cells = X_train.shape[0]
    min_expr_pct = 0.05
    min_cells = max(5, int(np.floor(n_cells * min_expr_pct)) + 1)
    genes_keep = X_train.columns[(X_train > 0).sum(axis=0) >= min_cells]
    return X_train[genes_keep], X_test[genes_keep]


def filter_xgb_params(params: dict) -> dict:
    valid = XGBClassifier().get_params().keys()
    return {k: v for k, v in params.items() if k in valid}


def load_all_results_new(result_dir: str, stem_to_celltype: dict):
    """
     results__*__test_geq0p6.pkl， key  stem  parquet  celltype
    """
    results_dict = {}
    pkl_files = sorted(
        glob.glob(os.path.join(result_dir, "results__*__test_geq0p6.pkl"))
    )
    for pkl_path in pkl_files:
        base = os.path.basename(pkl_path)
        stem_all = base[len("results__") : -len(".pkl")]
        cell_stem = stem_all.split("__")[0]
        if cell_stem not in stem_to_celltype:
            print(f"[SKIP] stem  parquet celltype: {cell_stem}")
            continue
        celltype = stem_to_celltype[cell_stem]
        with open(pkl_path, "rb") as f:
            results_dict[celltype] = pickle.load(f)
    return results_dict


def get_pair_result(results: dict, neg: str, pos: str):
    if (neg, pos) in results:
        return results[(neg, pos)], False
    if (pos, neg) in results:
        return results[(pos, neg)], True
    return None, None


def recompute_y_and_proba_from_parquet(
    parquet_path: str,
    celltype: str,
    neg: str,
    pos: str,
    ...
    data_cell = load_celltype_data(parquet_path, celltype)
    if data_cell.empty:
        return None

    split = prepare_pair_split(data_cell, neg, pos)
    if split is None:
        return None
    X_train, X_test, y_train, y_test = split

    X_train_f, X_test_f = gene_filter_like_original(X_train, X_test)

    feats = res_dict.get("selected_features_geq06", None)
    if feats is None and "boruta_stability" in res_dict:
        stab = res_dict["boruta_stability"]
        feats = stab.loc[stab["fold_freq"] >= 0.6, "gene"].tolist()
    if not feats:
        return None

    feats = [g for g in feats if g in X_train_f.columns]
    if len(feats) == 0:
        return None

    Xtr = X_train_f[feats].values
    Xte = X_test_f[feats].values

    params = filter_xgb_params(res_dict["final_params"])
    model = XGBClassifier(**params)
    model.fit(Xtr, y_train)

    pos_idx = np.where(model.classes_ == 1)[0][0]
    proba = model.predict_proba(Xte)[:, pos_idx]
    return y_test, proba


def plot_all_celltype_roc(
    results_dict,
    parquet_path,
    pair_classifiers=[("CON", "PRE"), ("CON", "T2D"), ("PRE", "T2D")],
    pair_names=["CON vs PRE", "CON vs T2D", "PRE vs T2D"],
    cell_type_list=None,
    color_list=None,
    save_prefix="allcelltype_ROC__test_geq0p6",
):
    ...
    if len(color_list) < len(cell_type_list):
        raise ValueError(
            "color_list : "
            f" {len(cell_type_list)} ， {len(color_list)}"
        )
    pal = dict(zip(cell_type_list, color_list))
    ...
    for idx, (class1, class2) in enumerate(pair_classifiers):
        ...
        for cell_type in cell_type_list:
            if cell_type not in results_dict:
                continue

            results = results_dict[cell_type]
            res, reverse = get_pair_result(results, class1, class2)
            if res is None:
                continue
            ...
            if y_test is None or y_proba is None:
                yp = recompute_y_and_proba_from_parquet(
                    parquet_path, cell_type, class1, class2, res
                )
                if yp is None:
                    continue
                y_test, y_proba = yp
            ...
            auc = res.get("test_auc_geq06", None)
            if auc is None:
                auc = roc_auc_score(y_test, y_proba)

            fpr, tpr, _ = roc_curve(y_test, y_proba)
            auc_list.append((auc, fpr, tpr, cell_type))
        ...
        for auc, fpr, tpr, cell_type in auc_list:
            plt.plot(
                fpr,
                tpr,
                color=pal[cell_type],
                linewidth=4.0,
                alpha=1,
                label=f"{cell_type} (AUC={auc:.4f})",
            )

        plt.plot([0, 1], [0, 1], "k--", linewidth=2.2)
        ...
        plt.xlabel("False Positive Rate", fontsize=font_big, fontweight="bold")
        plt.ylabel("True Positive Rate", fontsize=font_big, fontweight="bold")
        plt.title(
            f"{pair_names[idx]}",
            fontsize=font_big + 2,
            fontweight="bold",
        ...
        plt.tick_params(axis="both", which="major", labelsize=font_med, width=2)
        ...
        plt.legend(
            ...
            fancybox=True,
            edgecolor="gray",
            prop={"weight": "normal", "size": font_small},
        )

        plt.grid(True, alpha=0.22, linewidth=1.2)
        plt.tight_layout()
        plt.savefig(f"{save_prefix}_{class1}_vs_{class2}.pdf")
        plt.show()
...
if __name__ == "__main__":
    ...
    analyzer = PairwiseXGBSHAPCellAnalyzer(
        data_path=str(RNA_EXPRESSION_PARQUET),
